...
bool UInteractMLTrainingSet::BeginRecording(int label)
{
	...
	if (IsRecording())
	{
		UE_LOG(LogInteractML, Error, TEXT("Multiple simultaneous example recordings are not supported. You can only record from one node at a time into training set '%s'."), *GetFilePath() );
		return false;
	}
